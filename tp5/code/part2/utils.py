...
def normalize_adjacency(A):
    ...
    A_tilde = sp.identity(n) + A
    D_tilde = sp.diags(np.array(A_tilde.sum(axis=1)).flatten())
    D_tilde_inv_sqrt = sp.diags(np.power(np.array(D_tilde.sum(axis=1)).flatten(), -0.5))
